...
uint32_t tPWM_frequency[ntPWMs][FREQ_NUM_PER_PWM] = {{3000, 6000, 9000, 12000 ,17148 ,18379 ,24252 ,32000 ,16000}, {3000, 6000, 9000, 12000,17148 ,18379 ,24252 ,32000 ,16000}, {3000, 6000, 9000, 12000,17148 ,18379 ,24252 ,32000 ,16000}, {3000, 6000, 9000, 12000,17148 ,18379 ,24252 ,32000 ,16000}}; // 3-12 kHz
byte tPWM_duty[ntPWMs][FREQ_NUM_PER_PWM]          = {{128,  128, 128, 128 ,128 ,128 ,128 ,128 ,128}, {128,  128, 128, 128,128 ,128 ,128 ,128 ,128}, {128,  128, 128, 128,128 ,128 ,128 ,128 ,128}, {128,  128, 128, 128,128 ,128 ,128 ,128 ,128}}; // Deafault: 50% duty cycle

// Digital Input Enables
byte DigitalInputsEnabled[nDIs] = {1, 1, 1, 1};
bool LciksDetectionEnabled = 1;
...
byte smartFlag[5] = {0, 0, 0, 0, 0}; //smartFlag for Loop, smartFlag[0] is Noise flag, other flags will be defined in future
unsigned int SMARTCurrentTrialNum = 0;
/*****************************************************************/
...
void AlignmentOutput() {
  ...
  buf = queue1.getBuffer();
  if(buf) {
  memcpy(buf, block1, sizeof(block1));
  queue1.playBuffer();
  }
  buf = queue1.getBuffer();
  if(buf) {
  memcpy(buf, block2, sizeof(block2));
  queue1.playBuffer();
  }
  buf = queue1.getBuffer();
  if(buf) {
  memcpy(buf, block3, sizeof(block3));
  queue1.playBuffer();
  }
}
...
void gpSMART::init(uint8_t num_electrode){
  // init gpSMART hardware ports
  for (int x = 0; x < nDIs; x++) {
    pinMode(gpSMART_DI_Lines[x], INPUT);
  }
  ...
  Serial.begin(115200);
  // for action detection sensor
 // for capacitive sensor
  Wire.begin();
  Wire.setClock(400000); // MPR121 max rated speed is 400kHz
  cap.begin(num_electrode); // using num_electrode out of 12 electrode
...
void gpSMART::reinitCap(uint8_t num_electrode){
  cap.begin(num_electrode); // using num_electrode out of 12 electrode
...
void gpSMART::setLicksDetectionEnabled(bool LicksEnabled){
    LciksDetectionEnabled = LicksEnabled;
}
...
void gpSMART::ManualTimeAlignment(void) {
    AlignmentOutput();
    delay(18);
    mixer1.gain(0, 0);
    mixer1.gain(1, 1);
...
void gpSMART::TimeAlignmentInTrial(void) {
    AlignmentOutput();
}
...
void gpSMART::Run() {
  ...
  trial_res.stateTimeStamps[trial_res.nVisited - 1] = CurrentTime;
...
void gpSMART_Runner() {
  if (smartRunning) {
	...
	//  Lick action detectio
    if(LciksDetectionEnabled == 1){
    ...
    for (int x = 0; x < GLOBAL_TC_NUM; x++) {
      if (GlobalTimersActive[x] == true) {
        if (CurrentTime >= GlobalTimerEnd[x]) {
          CurrentEvent[nCurrentEvents] = Ev;
          nCurrentEvents++;
          // GlobalTimersActive[x] = false; // manual operation is the only way to disable the global timers
        }
      }
      Ev = Ev + 1;
    }
      ...
      if (sma.GlobalCounterAttachedEvents[x] < nInputs) {
        ...
        if(GlobalCounterCounts[x]){
          ...
          if (GlobalCounterCounts[x] > sma.GlobalCounterThresholds[x]) {
            CurrentEvent[nCurrentEvents] = Ev;
            nCurrentEvents++;
            GlobalCounterCounts[x] = 0; // inactive global counter
          }
        }
      }
      Ev = Ev + 1;
...
void setStateOutputs(byte iState) {
  ...
  for (int x = 0; x < nDOs; x++) {
    // Reserve DO3 for external/manual control (e.g., charging-warning fan) and
    // prevent state-machine refresh from overriding it.
    if (x == 2) {
      continue;
    }
  ...
  // Column 8~15: Regular PWM Output Line 1-nPWMs; Value: 0-255 TODO 为了让main loop来控制这三个led灯，这里只不会对 middle的灯的值有响应
  for (int x = 0; x < nPWMs; x++) {
    if(x != 2){
  ...
  if (sma.OutputMatrix[iState][nDOs + nPWMs + ntPWMs + 1] > 0) {
    // Spike recording
    Serial.write("ModeSwitch:");                         
    if(sma.OutputMatrix[iState][nDOs + nPWMs + ntPWMs + 1] == 2){
      Serial.write("ESA"); 
    }else if (sma.OutputMatrix[iState][nDOs + nPWMs + ntPWMs + 1] == 1){
      Serial.write("LFP"); 
    }
    Serial.println();
  }
  ...
  if (CurrentCounter > 0 && CurrentCounter <= GLOBAL_TC_NUM) {
    ...
    GlobalCounterCounts[CurrentCounter] = 1;
  }
  // Column 25-28: smartFlag Output 1-5; Value: 0 or 1
  for (int x = 0; x < nFlags; x++) {
    smartFlag[x] = sma.OutputMatrix[iState][x + nDOs + nPWMs + ntPWMs + 5];
  }

  // Column 29: DAC1 output: sine waveform; amp is fixed to 1V; frequency: 5000Hz
    byte DAC1_sine_frequency = sma.OutputMatrix[iState][nDOs + nPWMs + ntPWMs + nFlags + 5];
    if (DAC1_sine_frequency > 0) {
        mixer1.gain(0, 0);
        mixer1.gain(1, 1);
      // AlignmentOutput();
      // Serial.print((String)"C:"+ (SMARTCurrentTrialNum+1)); // Clock SMARTCurrentTrialNum + 1
    }
    //  else {
    //   mixer1.gain(0, 0);
    //   mixer1.gain(1, 1);
