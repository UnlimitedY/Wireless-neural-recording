...
#define MAX_STATE_NUM     128    // max number of state in gpSMART
#define GLOBAL_TC_NUM     3     // number of Timer/Counter in gpSMART
#define SOFT_EVENT_NUM    4     // number of Soft events
#define FREQ_NUM_PER_PWM  9     // number of frequencies per true PWM
...
extern int16_t block0[128]; 
extern int16_t block1[128]; 
extern int16_t block2[128];   
extern int16_t block3[128];   
...
const PROGMEM String EventNames[] = { // 25 in total = 2 * (nDIs + nLicks) + SOFT_EVENT_NUM + 1 + 2 * GLOBAL_TC_NUM
  ...
  "GlobalTimer3_End", 	// Global Timer 3 is up

  "GlobalCounter1_End", // Global Counter 1 reached threshold
  "GlobalCounter2_End", // Global Counter 2 reached threshold
  "GlobalCounter3_End"  // Global Counter 2 reached threshold

};

const PROGMEM String OutputActionNames[] = { // 29 in total = 8 nDOs + 8 nPWMs + 4 ntPWMs + 2 Serial + 3 Global + 5 Flag
  ...
  "Flag1",		// Flag1 for Loop
  "Flag2",		// Flag2 for Loop
  "Flag3",		// Flag3 for Loop
  "Flag4",		  // Flag4 for Loop
  "Flag5",
...
const byte nOutputs = sizeof(OutputActionNames) / sizeof(OutputActionNames[0]); // 29
...
struct StateMatrix {
  ...
  byte GlobalCounterAttachedEvents[GLOBAL_TC_NUM]        = {nInputs, nInputs, nInputs};	// Default event of nInputs is code for "no event attached".
...
struct TrialResult {
  ...
  unsigned long stateTimeStamps[MAX_VISIT_NUM]  = {};
};
...
extern byte smartFlag[5]; // flags1-4 for loop
// 设置DAC1为正弦波输出，作为电磁脉冲 时间对齐基准
extern AudioSynthWaveformSine   sine1;          //xy=139,82
extern AudioOutputAnalog        dac1;           //xy=322,90
...
class gpSMART {
  public:
    ...
    gpSMART();
    // Fucntions
    void init(uint8_t num_electrode);
    void reinitCap(uint8_t num_electrode);
    void setDigitalInputsEnabled(byte* PortEnabled);
    void setLicksDetectionEnabled(bool LicksEnabled);
    ...
    void ManualTimeAlignment();
    void TimeAlignmentInTrial();
